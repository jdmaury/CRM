	...
	def salvarAnexo(ReqLotusSw reqLotusSwInstance){
		...
		try {
			...
			log.error InetAddress.getLocalHost().getHostAddress()+" - "+usuarioAccede + "  - Error al crear el requerimiento al usar el servicio reqCreacion "
			 ...
			 log.error InetAddress.getLocalHost().getHostAddress()+" - "+usuarioAccede + "Descripci贸n "+  params.descripcionReq
		...
		if(rta){
			...
			def xtitulo = generalService?.getValorParametro('opttyReq01')
			flash.message = ("Error, no fue posible la creaci贸n del Requerimiento")
			def numOportunidad  = params.numOptty
					...
					if (params.hayAnexo=='S'){
						try {
							...
							try {
								...
								String id = "SR-0000"  // "Informaci贸n no requerida pra crear el anexo"
								...
								//log.info InetAddress.getLocalHost().getHostAddress()+ " Informaci贸n antes de procesar el anexo, El numero de requerimiento y la ruta del adjunto" + autor + " -- "  +  archivo
										...
										if (params.hayAnexo1=='S'){
											try {
												...
												try {
													...
													//log.info InetAddress.getLocalHost().getHostAddress()+ " Antes de procesar el 2 adjunto informaci贸n de # req y adjunto " + datoReq + " -- Aarchivo "+ archivo
	...
	def index(Integer max) {
		...
		if (params.pedido =='Si'){
			try {
			   ...
			   //log.error InetAddress.getLocalHost().getHostAddress()+" - "+usuarioAccede + "  - Errar al mostrar el requerimiento para la oportunidad -- "+ numOptty +"- URL - " + urlCompleta + " - Opci贸n 3- "
   ...
   **/
	...
	**/
	...
	def sendJson=
	{
		...
		{
			idClienteOptty=Pedido.findByNumPedido(params.numOptty).empresa.nit.toString().split("-")[0]
		...
		http.request(POST) {
			...
			body = [ "DescripcionTema":descripcionTema,
			 ...
			 "nombreContactoOptty":"",
			 "nombreContactoIT":"", //ESTE CAMPO SE ENV虯 VAC蚈
			...
			println "DescripcionTema "+descripcionTema
				...
				println "nombreContactoIT "+"" //ESTE CAMPO SE ENV虯 VAC蚈
